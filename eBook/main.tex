\usepackage{lipsum}
\usepackage[automake,toc]{glossaries}
\usepackage{caption}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{filecontents}
\makeglossaries
% Term definitions 

\begin{document}

% =======================================================================================
%\cleardoublepage % Forces the first chapter to start on an odd page so it's on the right

% =======================================================================================
%                                   PREAMBLE
% =======================================================================================
\coverpage{\TITLE}{\SUBTITLE}{\AUTHOR}{\DATE}{\SUBJECT}
%----------------------------------------------------------------------------------------
\newpage
\tableofcontents

% =======================================================================================
%                                   PART I
% =======================================================================================
\part{Introduction to Cardano Smart Contracts}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Overview of Cardano Blockchain} \label{ch:overview}
\input{sections/part1/overview}
% =======================================================================================
%                                   PART II
% =======================================================================================
\part{Setting Up the Development Environment}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Installing and Configuring Cardano Development Tools} \label{ch:setup}
\input{sections/part2/setup}

% =======================================================================================
%                                   PART III
% =======================================================================================
\part{Exploring the eUTxO Model}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Understanding the eUTxO Model and Its Components} \label{ch:exploring}
\input{sections/part3/exploring}


\part{Cardano Smart Contract Languages}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Cardano Smart Contract Languages} \label{ch:Languages}
\input{sections/part4/languages}

\part{Smart Contract\\ Runtime}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Smart Contract Runtime and Execution} \label{ch:Runtime}
\input{sections/part5/runtime}

\part{Write your first\\ Smart Contract}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Getting Started with Cardano Smart Contract Development} \label{ch:Getting started}
\input{sections/part6/gettingstarted.tex}

\part{Building a Marketplace\\ Smart Contract}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Building a Marketplace} \label{ch:Building a Marketplace}
\input{sections/part7/marketplace.tex}

\part{On-Chain Governance}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Governance Smart Contracts} \label{ch:Governance}
\input{sections/part8/governance.tex}

\part{Advanced Techniques \\ and Design Patterns}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Advanced Smart Contract Design Patterns} \label{ch:AdvancedPatterns}
\input{sections/advanced/advancedpatterns.tex}

\part{Conclusion and \\ Next Steps}
%----------------------------------------------------------------------------------------
\newpage
\chapter{Conclusions} \label{ch:Conclusions}
\input{sections/part9/conclusions.tex}

\newpage
\part{EXERCISE solutions}
\chapter{Solutions}
\input{sections/solutions/solutions.tex}


\input{sections/glossary/glossary.tex}
\printglossary[title={Glossary}, type=main]
\end{document}


%https://iohk.io/en/blog/posts/2021/02/02/plutus-tx-compiling-haskell-into-plutus-core/ reference plutus tx